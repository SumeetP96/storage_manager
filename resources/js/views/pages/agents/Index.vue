<template>
  <div>
    ...
    <v-col cols="12" lg="11" class="mx-auto">
      ...
      <div class="text-h5 py-4">Agents</div>
      ...
      <v-row align="end">
        ...
        <v-col cols="12" sm="12" md="6" class="text-h5 d-flex">
          ...
          <!-- New record -->
          <v-btn color="indigo" dark :to="{ name: 'agents.action' }"
            v-shortkey="['alt', 'n']" @shortkey.once="$router.push({ name: 'agents.action' })">
          ...
          <v-btn tabindex="-1" style="width: 120px" :disabled="disableExport || records.length == 0"
            ...
            :href="`/exports/pdf/agents?query=${query}&sortBy=${sortBy}&flow=${flow}&${customQuery}`"
          ...
          <v-btn tabindex="-1" class="ml-2" style="width: 120px" :disabled="disableExport || records.length == 0"
            ...
            :href="`/exports/excel/agents?query=${query}&sortBy=${sortBy}&flow=${flow}&${customQuery}`"
          ...
          :loading="refreshLoading"
            ...
            :disabled="disableExport || records.length == 0" @click="disableExportButtons();
              printPage('all-print', `/exports/print/agents?query=${query}&sortBy=${sortBy}&flow=${flow}&${customQuery}`)">
        ...
        <v-card-actions class="d-flex justify-space-between">
          <div class="d-flex">
            <v-btn :color="$vuetify.theme.dark ? 'primary' : 'indigo'" text
              @click="editFromTable({ name: 'agents.action', params: { id: record.id } })">
                <v-icon class="text-h6 mr-2">mdi-circle-edit-outline</v-icon> edit agent
            </v-btn>
            ...
            <v-btn color="error" text tabindex="-1" :disabled="disableExport"
              ...
              :href="`/exports/pdf/agents/${record.id}`"
              :download="`${apiRoute}.pdf`">
                <v-icon class="text-h6 mr-2">mdi-file-pdf</v-icon> PDF
            </v-btn>

            <!-- Print -->
            <v-btn @click="disableExportButtons(2); printPage('print-record', `/exports/print/agents/${record.id}`)"
