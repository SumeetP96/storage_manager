<template>
  <div>
    ...
    <v-col cols="12" lg="11" class="mx-auto">
      ...
      <div class="text-h5 py-4">Inter Godown Transfers</div>
      ...
      <v-row align="end">
        ...
        <v-col cols="12" sm="12" md="6" class="text-h5 d-flex">
          ...
          <v-btn color="indigo" dark :to="{ name: 'inter_godowns.action' }"
            v-shortkey="['alt', 'n']" @shortkey.once="$router.push({ name: 'inter_godowns.action' })">
          ...
          <v-btn tabindex="-1" style="width: 120px" :disabled="disableExport || records.length == 0"
            ...
            :href="`/exports/pdf/inter_godowns?query=${query}&sortBy=${sortBy}&flow=${flow}&${customQuery}`"
          ...
          <v-btn tabindex="-1" class="ml-2" style="width: 120px" :disabled="disableExport || records.length == 0"
            ...
            :href="`/exports/excel/inter_godowns?query=${query}&sortBy=${sortBy}&flow=${flow}&${customQuery}`"
          ...
          <v-btn tabindex="-1" class="ml-2" style="width: 120px"
            ...
            :disabled="disableExport || records.length == 0" @click="disableExportButtons();
              printPage('all-print', `/exports/print/inter_godowns?query=${query}&sortBy=${sortBy}&flow=${flow}&${customQuery}`)">
          ...
          <template v-slot:default>
            <thead>
              <tr>
                ...
                <!-- Transfer no -->
                <th class="subtitle-2" :class="sortBy == 'inter_godown_no' ? 'pink--text font-weight-bold' : ''"
                  :style="sortBy == 'inter_godown_no' ? 'font-size: 1rem !important' : ''">
                    <span class="sort-link" @click="sortRecords('inter_godown_no', sortBy)">Transfer no</span>
                    <span v-if="sortBy == 'inter_godown_no'">
                      <span v-if="flow =='asc'"><v-icon class="subtitle-1 pink--text">mdi-arrow-down</v-icon></span>
                      <span v-else><v-icon class="subtitle-1 pink--text">mdi-arrow-up</v-icon></span>
                    </span>
                </th>
            ...
            <tbody v-if="records.length > 0">
              <tr v-for="record in records" :key="record.name" style="cursor: pointer"
                @click="viewRecordDialog = true; customFetchTransfer(record.id)">
                  ...
                  <td class="subtitle-1 grey--text font-weight-bold"
    ...
    <v-dialog v-model="viewRecordDialog" max-width="90%">
      <v-card>
        <v-card-title class="headline d-flex justify-space-between align-center">
          <div>Inter Godown Transfer Details</div>
          ...
          <v-row no-gutters>
            ...
            <v-col cols="3" class="px-2">
              <div class="px-3 py-1" :class="$vuetify.theme.dark ? 'grey darken-3' : 'grey lighten-4'">
                <div class="font-weight-bold grey--text" :class="$vuetify.theme.dark ? '' : 'text--darken-1'">Transfer no</div>
                <div class="px-4 py-1">
                  <div class="font-weight-bold">{{ record.inter_godown_no }}</div>
                </div>
              </div>
            </v-col>
        ...
        <v-card-actions class="d-flex justify-space-between">
          <div class="d-flex">
            ...
            <v-btn color="error" text tabindex="-1" :disabled="disableExport"
              ...
              :href="`/exports/pdf/inter_godowns/${record.id}`"
              :download="`${apiRoute}.pdf`">
                <v-icon class="text-h6 mr-2">mdi-file-pdf</v-icon> PDF
            </v-btn>

            <!-- Print -->
            <v-btn @click="disableExportButtons(2); printPage('print-record', `/exports/print/inter_godowns/${record.id}`)"
...
export default {
  ...
  methods: {
    ...
    fetchToGodownAutofill() {
      this.filterLoading = true
      this.axios.get('/api/autofills/godowns/to_with_transactions/1')
        .then(response => {
    ...
    fetchFromGodownAutofill() {
      this.filterLoading = true
      this.axios.get('/api/autofills/godowns/from_with_transactions/1')
        .then(response => {
